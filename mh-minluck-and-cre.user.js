...
function shouldRender() {
	return document.getElementsByClassName('trapImageView-trapAuraContainer')[ 0 ] && document.getElementById('mousehuntContainer').className.includes('PageCamp');
}
...
function addStyles() {
	...
	#chro-minluck-table th {
		text-align: center;
		font-weight: bold;
	}

	#chro-minluck-table th.mousename-header {
		text-align: left;
	}

	#chro-minluck-table:first-child {
		text-align: left;
	}
...
const allType = ['Arcane', 'Draconic', 'Forgotten', 'Hydro', 'Parental', 'Physical', 'Shadow', 'Tactical', 'Law', 'Rift'];

function render() {
	if (document.getElementById('min-luck-btn')) {
		return;
	}

	const minluckButton = document.createElement('a');
	minluckButton.id = 'min-luck-btn';
	minluckButton.classList.add('campPage-trap-trapEffectiveness');
	minluckButton.textContent = '🐭️ Minluck & Catch Rate Estimate';
	minluckButton.style.marginTop = '5px';

	$('.campPage-trap-statsContainer').append(minluckButton);
...
function getData() {
	return new Promise((resolve) => {
		// eslint-disable-next-line no-undef
		postReq('https://www.mousehuntgame.com/managers/ajax/users/getmiceeffectiveness.php', `sn=Hitgrab&hg_is_ajax=1&uh=${ user.unique_hash }`
		).then(async (res) => {
			try {
				const response = JSON.parse(res.responseText);
				if (response) {
					const effect = ['Effortless', 'Easy', 'Moderate', 'Challenging', 'Difficult', 'Overpowering', 'Near Impossible', 'Impossible'];
					const temList = [];
					for (let i = 0; i < effect.length; i++) {
						if (response.effectiveness[ effect[ i ] ]) {
							for (let j = 0; j < response.effectiveness[ effect[ i ] ].mice.length; j++) {
								temList.push(response.effectiveness[ effect[ i ] ].mice[ j ].name);
							}
						}
					}

					await renderBox(temList)
						.then(() => {
							sortTable();
						});
					resolve();
...
function renderBox(list) {
	return new Promise((resolve) => {
		...
		const power = Number(document.getElementsByClassName('campPage-trap-trapStat power')[ 0 ].children[ 1 ].innerText.match(/[0-9]/g).join(''));
		const luck = Number(document.getElementsByClassName('campPage-trap-trapStat luck')[ 0 ].children[ 1 ].innerText);
		const powerType = document.getElementsByClassName('campPage-trap-trapStat power')[ 0 ].children[ 1 ].innerText.match(/[a-zA-Z]+/g)[ 0 ];
...
function sortTable() {
	const table = document.getElementById('chro-minluck-table');
